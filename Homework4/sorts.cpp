...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
	...
	for (int unsorted = 1; unsorted < s.size(); unsorted++)
	{
		...
		int loc = unsorted;
		while ((loc > 0) && compareSensor(nextItem, s[loc - 1]))
		{
			// Shift s[loc - 1] to the right
			s[loc] = s[loc - 1];
			loc--;
		} // end while
			 // At this point, s[loc] is where nextItem belongs
